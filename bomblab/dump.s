...
0000000000400efc <phase_2>:
  ...
  # (%rsp) 开始的 24 bytes (6 int) 保存输入的六个数字
...
0000000000400f43 <phase_3>:
  ...
  # 0x8(%rsp) 第一个数 0x10(%rsp) 第二个数
  # 0 <= 第一个数 <= 7
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a> # boom
  # 根据第一个数的值，可选跳转到以下八个地址
...
0000000000400fce <func4>: # 参数为：[rdi=第一个输入(inp), rsi=0, rdx=14]，应当返回 0
  400fce:	48 83 ec 08          	sub    $0x8,%rsp
  400fd2:	89 d0                	mov    %edx,%eax # ax = 14
  400fd4:	29 f0                	sub    %esi,%eax # ax = 14 - 0 = 14
  400fd6:	89 c1                	mov    %eax,%ecx # cx = 14
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx # cx = cx >> 31 (logical) = 0
  400fdb:	01 c8                	add    %ecx,%eax # ax = ax + cx = 14
  400fdd:	d1 f8                	sar    %eax # ax = ax >> 1 (arithmetic) = 7
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx # cx = 7
...
0000000000401062 <phase_5>:
  ...
  # --- key code
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx # ax = 0..5, bx = &str
  40108f:	88 0c 24             	mov    %cl,(%rsp) # 第0..5个字符
  401092:	48 8b 14 24          	mov    (%rsp),%rdx # 第0..5个字符
  401096:	83 e2 0f             	and    $0xf,%edx # 第0..5个字符的低 4 位
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx
  # 由第0..5个字符的低 4 位 ascii 选择 edx 的值 (9 15 14 5 6 7)
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1)
  # 选择结果放在 rsp + 10 + 0..5 上
  4010a4:	48 83 c0 01          	add    $0x1,%rax
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax
  4010ac:	75 dd                	jne    40108b <phase_5+0x29>
  # --- end key code (runtime = 6)
...
00000000004010f4 <phase_6>:
  ...
  40110b:	49 89 e6             	mov    %rsp,%r14 # 第一个数的地址
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d
  # 输入六个数字 保存在栈上 x/24x $rsp
  ...
  # 将第 2..5 个数与第一个比较，不同则跳
  401140:	e8 f5 02 00 00       	call   40143a <explode_bomb>
  # --- end
  401145:	83 c3 01             	add    $0x1,%ebx
  401148:	83 fb 05             	cmp    $0x5,%ebx
  40114b:	7e e8                	jle    401135 <phase_6+0x41>
  40114d:	49 83 c5 04          	add    $0x4,%r13 # r13 向后指一位
  401151:	eb c1                	jmp    401114 <phase_6+0x20>
  # 前面保证 六个数均位于 [1, 6]，且两两不同
  ...
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa> # key
...
000000000040145c <read_six_numbers>:
  ...
  # __isoc99_sscanf 返回成功匹配并赋值的输入项数量
